...
//	max30102_Bus_Write(REG_LED2_PA, 0x47); 
	...
	// max30102_Bus_Write(REG_INTR_ENABLE_1, 0xc0); // 开启中断
    // max30102_Bus_Write(REG_FIFO_CONFIG, 0x5F);   // 【关键】允许 FIFO 滚动覆盖，防止溢出挂起
    // max30102_Bus_Write(REG_MODE_CONFIG, 0x03);   // SpO2 模式
    
    // // 量程调至最大 (16384nA)，分辨率设为 15位 (提高动态范围)
    // max30102_Bus_Write(REG_SPO2_CONFIG, 0x61);   //

    // // 亮度降至极低 (约 0.4mA)，防止直接照盲传感器
    // max30102_Bus_Write(REG_LED1_PA, 0x02);       // 红光
    // max30102_Bus_Write(REG_LED2_PA, 0x02);       // 红外
...
{
    max30102_Bus_Write(REG_MODE_CONFIG, 0x40); // 发送复位命令
    delay_ms(50);                             // 【必须】等待硬件复位完成
    max30102_Bus_Write(REG_MODE_CONFIG, 0x40); // 发送复位命令
}
...
{		
    ...
    MAX30102_IIC_Send_Byte(deviceAddr);	    // 发送器件地址（写）
    MAX30102_IIC_Wait_Ack();
    MAX30102_IIC_Send_Byte(writeAddr);      // 发送寄存器地址
    MAX30102_IIC_Wait_Ack();

    MAX30102_IIC_Start();                   // 【必须添加】重复起始信号！
    MAX30102_IIC_Send_Byte(deviceAddr|0X01);// 发送器件地址（读）
    MAX30102_IIC_Wait_Ack();
    
    for(i=0; i<dataLength-1; i++)
    {
        data[i] = MAX30102_IIC_Read_Byte(1); // 发送 ACK
    }		
    data[dataLength-1] = MAX30102_IIC_Read_Byte(0); // 最后一个字节发送 NACK
    
    MAX30102_IIC_Stop();
    delay_us(5); // 增加短暂延时保证总线释放
}
